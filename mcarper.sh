...
${TEMP_FILE}
  ...
  (
    ...
    then
      ...
      if [[ $( echo "${WEB_BLK}" | grep -Fc '"backend":' ) -gt 0 ]] && [[ $( echo "${WEB_BLK}" | grep -Fc '"blocks":' ) -gt 0 ]]
      then
        WEB_BLK=$( echo "${WEB_BLK}" | jq -r '.backend.blocks' 2>/dev/null )
      fi

      if [[ ! $( echo "${WEB_BLK}" | tr -d '[:space:]') =~ $RE ]] && [[ $( echo "${WEB_BLK}" | grep -c 'data' ) -gt 0 ]]
      then
  ...
  (
    ...
    then
      ...
      ADDNODE_URL="https://www.dropbox.com/scl/fi/${DROPBOX_ADDNODES}/peers_1.txt?rlkey=hcach9px7tbsq9hon83i68xvb&dl=0"
    fi
